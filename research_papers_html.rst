...
Supplementary materials, figures, tables, etc. are kept as separate downloads in HTML. When possible, a single file is used for each, but it is OK if tables and figures appear in separate files (regardless of size). Supplementary references should be kept inside the file that they are cited in. If they are being cited in more than one file, then they should be kept in their own file.
